...
## 1. Micro-SD-Karte formatieren  
Für die Installation des Betriebssystems auf dem Raspberry Pi benötigen Sie eine Micro-SD-Karte mit mindestens 8 GB (besser 16 GB), die zuerst formatiert werden sollte. Für die Formatierung der SD-Karte mithilfe eines Windows PC sind die nachfolgenden Schritte durchzuführen:

* **Laden** Sie sich den 
[SD Memory Card Formatter](https://www.sdcard.org/downloads/formatter_4/)
herunter und **installieren** Sie diesen auf Ihrem PC.

* **Stecken** Sie die **Micro-SD-Karte** (unter Umständen Adapter notwendig) in den vorgesehen **Steckplatz** Ihres PCs oder nutzen Sie einen entsprechenden Adapter und führen Sie den **SD Memory Card Formatter** aus.  
 ![](images/commissioning/SD_memory_card_formatter.png)
* Wählen Sie die SD-Karte unter **Select Card** und unter **Formatting Options** den Punkt **Overwrite format** aus. Unter **Volume label** können Sie einen neuen Namen für die Micro-SD-Karte vergeben.
* Führen Sie die Formatierung der Micro-SD-Karte mit **Format** durch und bestätigen Sie abschließend die erfolgreiche Formatierung mit **OK**.

## 2. Micro-SD-Karte mit Raspian Betriebssystem beschreiben
...
![](images/commissioning/welcome_to_raspberry_pi.png)
 ...
 5. Als erstes sind die Einstellung für  **Tastatur**, **Sprache** und **Zeitzone** wie in der nachfolgenden Abbildung dargestellt vorzunehmen und mit **Next** zu bestätigen.  
 ...
 9. Im nächsten Schritt muss das **Betriebssystem** durch Klicken auf **Next** **aktualisiert** werden (Internetverbindung notwendig). Bestätigen Sie die erfolgreiche Aktualisierung mit **OK**.  
 ...
 11. Für die Kommunikation des Raspberry Pi mit verschiedenen **Sensoren** sind noch Schnittstellen zu aktivieren. Hierzu rufen Sie über den Reiter **Einstellungen** die **Raspberry Pi Konfiguration** auf, aktivieren unter **Schnittstellen SPI** und **I2C** und bestätigen die Konfiguration mit **OK**.  
 ![](images/commissioning/raspi_config.png)
  &nbsp;  
 ![](images/commissioning/activating_spi_i2c.png)
 
## 4. Fernzugriff (VNC) einrichten
...
### 4.1 Raspberry Pi für VNC konfigurieren 

 1. **Ändern** Sie den **Hostnamen** des Raspberry Pi, indem Sie über den Reiter **Einstellungen** die **Raspberry Pi Konfiguration** aufrufen und den Hostnamen bspw. in Raspi plus eine fortlaufende Nummer (Raspi1,...) ändern.  
![](images/commissioning/raspi_config.png)
&nbsp;  
...
![](images/commissioning/activating_vnc.png)
 3. Die **erfolgreiche Aktivierung** des VNC-Servers lässt oben rechts auf dem Bildschirm durch das neu erschienene **VNC-Sysmbol** erkennen.  
![](images/commissioning/vnc_activated.png)

### 4.2 Mobilen WLAN-Router konfigurieren
...
### 4.3 VNC-Viewer auf dem Anzeigegerät  einrichten
...
4. **Authentifizieren** sich mit dem **Benutzer pi** und dem jeweiligen **Passwort** und bestätigen Sie mit **OK**.  
...
## 5. Bildschirmauflösung an das Anzeigegerät anpassen
...
1. Öffnen Sie die **Kommandozeile** und wechseln sie mit dem nachfolgenden Befehl in die **Konfiguration** des Raspberry Pi.  
...
![](images/commissioning/sudo_raspi_config.png)  
 ...
 2. Wählen Sie Punkt 7 **Advanced Options** aus.  
 ![](images/commissioning/advanced_options.png)  
 
 3. Wählen Sie Punkt A5 **Resolution** aus.  
 ![](images/commissioning/resolution.png)  
 
 4. Unter den aufgeführten **Auflösungen** ist nun eine Auflösung zu **wählen** und mit **OK** zu bestätigen.  Die zu wählende Auflösung richtet sich nach dem verwendeten Anzeigegerät und sollte so festgelegt werden, dass an diesem angenehm gearbeitet werden kann. Für einen Laptop mit 14 Zoll Bildschirmdiagonale bietet sich bspw. die Auflösung **DMT Mode 85 1280x720 60Hz 16:9** an.  
![](images/commissioning/choose_screen_resolution.png)  

5. Beenden Sie die Konfiguration über **Finish**.  
![](images/commissioning/finish_config.png)  

6. Anschließend muss der Raspberry Pi über **Ja** neu gestartet werden, um die vorgenommene Änderung wirksam zu machen.  
![](images/commissioning/reboot.png)  

## 6. Installation PhyPiDAQ
Bevor die eigentliche Installation des Softwarepakets **PhyPiDAQ** erfolgt, wird der Raspberry Pi über die **Kommandozeile** (siehe 5.) auf den aktuellen Stand gebracht. Dazu werden die folgenden Befehle ausgeführt:

	sudo apt-get update
	sudo apt-get upgrade
Außerdem sind einige Software-Pakete zu installieren:
...
Im Anschluss wird noch das Softwarepaket **PhyPiDAQ** heruntergeladen und Bibliotheken für verwendete Sensoren installiert:
...
## 7. Systemabbild als Backup und für die Installation mehrerer Raspberry Pi mit identischer Konfiguration
Um nicht für jeden einzurichtenden Raspberry Pi alle Konfigurationsschritte durchführen zu müssen und eine Sicherung des konfigurierten Zustandes vorzunehmen, ist es sinnvoll, ein Systemabbild (Image) zu erzeugen und zu sichern. Da die meisten Lehrer und Schulen mit Windows Computern arbeiten, wird das Vorgehen anhand dieses Betriebssystems erläutert.
 ...
 2. **Führen** Sie den **Win32-Disk-Imager** aus und legen Sie eine **Image-Datei** an. Wählen Sie unter Datenträger die Micro-SD-Karte aus und starten sie das Erzeugen des Systemabbilds über **Lesen**.  ![](images/commissioning/win32_disk_manager_reading.PNG)  
 3. Das Erzeugen des Systemabbilds nimmt einige Minuten in Anspruch. Bestätigen Sie das erfolgreiche Erzeugen das Systemabbild mit **OK**. Sie können nun Micro-SD-Karten mit diesem Systemabbild beschreiben und sich so eine bestimmte Konfiguration eines Raspberry Pi klonen bzw. einen Raspberry Pi auf diesen Zustand zurücksetzen.  ![](images/commissioning/win32_disk_manager_reading_successful.PNG)
 

## 8. Systemabbild auf Micro-SD-Karte schreiben
Nachfolgend wird beschrieben, wie Sie ein gesichertes **Systemabbild (Image)** auf eine Micro-SD-Karte **schreiben** können.
Dies ist auch dann  sinnvoll, wenn Sie weitere Raspberry Pi mit **identischer Grundkonfiguration** verwenden wollen. Hier bietet es sich an, ein zuvor erstelltes **Systemabbild** (siehe 7.) eines einzelnen Raspberry Pi auf eine entsprechende Anzahl an **Micro-SD-Karten** zu **klonen**, um sich den Aufwand des mehrmaligen Konfigurierens zu ersparen. 
 ...
 3. Wenn Sie einen Raspberry Pi nur auf einen zuvor gesicherten Systemzustand zurücksetzen wollen, sind Sie an dieser Stelle fertig. Wollen Sie hingegen ein **Systemabbild klonen** und auf einem anderen Raspberry Pi verwenden, muss noch der **Hostnamen** geändert werden **(siehe 4.1)**. Wenn Sie von einem anderen Gerät über VNC auf den Raspberry Pi zugreifen wollen, sollten Sie dem geklonten Raspberry Pi außerdem in der **Konfiguration des WLAN-Routers** eine **feste IP-Adresse** zuweisen **(siehe 4.2)**.